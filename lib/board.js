  ...
  var isWindows = process.platform === 'win32';
	...
	if (isWindows) {
		serial.list(function (err, results) {
			var possible = results,
				found = false;
			for (var i in possible) {
				if (possible[i].manufacturer.indexOf("arduino") > -1) {
					var portname = possible[i].comName;
					var tempSerial, err;
					try {
						tempSerial = new serial.SerialPort(portname, {
							baudrate: 115200,
					...
					if (!err) {
						found = tempSerial;
						self.log('info', 'found board at ' + portname);
						break;
					}
				}
			}
